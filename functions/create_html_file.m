function [ output_args ] = create_html_file( filename, config, hyperbola_lat, hyperbola_long, heatmap_cell, heatmap_threshold )
%CREATE_HTML_FILE   Generates the html code for google maps showing RX
%                   positions and the hyperbolas
%
%   config:	    config file with lis of lat/long for RX positions (for displaying receiver positions)
...
%   heatmap_cell:   cell array, that contains {1} = long vector, {2} = lat vector and {3} = magnitude of heatmap points vector
    ...
    num_rx_positions = config.num_rx;
	...
	'  var mapTypeIds = [];'...
        ...
        num2str(config.MAP.lat) ', ' num2str(config.MAP.long) ...
        '); \n'...
        '  var mapCanvas = document.getElementById("map"); \n'...
        '  var mapProp = {center: myCenter, zoom: '...
        int2str(config.MAP.zoom)...
        ', scaleControl: true, mapTypeId: "OSM",'...
    ...
    extra = { ""; ""; ""; ", icon: 'brown_MarkerT.png'" };

    %% write RX positions to html file
    for ii = 1:num_rx_positions
        fprintf(fid, '\n');
        fprintf(fid, ['  var rx' num2str(ii) '_MarkerCenter = new google.maps.LatLng(' num2str(config.(['RX' num2str(ii)]).lat, 8) ', ' num2str(config.(['RX' num2str(ii)]).long, 8) '); \n']);
        fprintf(fid, ['  var marker_rx' num2str(ii) ' = new google.maps.Marker({position: rx' num2str(ii) '_MarkerCenter' extra{ii} '}); \n']);
        fprintf(fid, ['  marker_rx' num2str(ii) '.setMap(map); \n\n']);
    end;

    if isfield(config, "TRU")
        fprintf(fid, '\n');
        fprintf(fid, ['  var tru_MarkerCenter = new google.maps.LatLng(' num2str(config.TRU.lat, 8) ', ' num2str(config.TRU.long, 8) '); \n']);
        fprintf(fid, ['  var marker_tru = new google.maps.Marker({position: tru_MarkerCenter, icon: ''' config.TRU.icon '''}); \n']);
        fprintf(fid, ['  marker_tru.setMap(map); \n\n']);
    end;
    ...
    for ii_hyperb = 1:num_hyperbolas
        fprintf(fid, ['  var flightPath' num2str(ii_hyperb) '= new google.maps.Polyline({\n'...
                      ...
                      '    strokeColor:' char(config.hyperbol_color(ii_hyperb)) ',\n' ...
                      '    strokeOpacity:0.6,\n'...
                      '    strokeWeight:3\n'...
                      '  });\n'...
                      '  \n'...
